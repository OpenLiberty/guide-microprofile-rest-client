...
Then, add two more annotations to the `src/main/java/io/openliberty/guides/inventory/client/SystemClient.java` file so that the interface annotations look like the following:
...
Similarly, you will invoke the `system` service by calling the `customRestClient.getProperties()` method.
