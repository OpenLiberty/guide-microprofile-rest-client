...
You will learn how to build a MicroProfile Rest Client to access remote RESTful services. MicroProfile Rest Client provides a type-safe way to invoke RESTful services.
...
You will learn how to create a template interface that maps to the remote service that you want to call.
<<<<<<< HEAD
The MicroProfile Rest Client API automatically generates an instance of the template interface based on its annotations, so that you don't waste time on setting up the client and connecting with the remote service.
...
When the CDI injection is available for you and the base URL is static and able to configure, you can inject the client with CDI easily. When the CDI option is not available, use the more verbose `RestClientBuilder` method.
In this guide, you will explore both methods in order to handle different scenarios to provide a valid base URL.
=======
The MicroProfile Rest Client API automatically generates an instance of the template interface based on its annotations. Thus, you don't waste time on setting up the client and connecting with the remote service.

You will instantiate the client and use it in the `inventory` service. You can choose from two different approaches, Context and Dependency Injection (CDI) with the help of MicroProfile Config or the RestClientBuilder method.
When the CDI injection is available for you, and the base URL is static and able to configure, you can inject the client with CDI easily. When the CDI option is not available, use the more verbose `RestClientBuilder` method.
In this guide, you will explore both methods to handle different scenarios for providing a valid base URL.
>>>>>>> 873335477642d58f1db817e835d3c040a49711ab
...
* The `http://localhost:9080/inventory/systems/{your_hostname}` microservice is the `inventory` service that invokes the `http://{your_hostname}:9080/system/properties` microservice. Get your fully qualified domain name (FQDN), visit the URL by replacing `{your_hostname}` with your FQDN. 
...
Next, create a RESTful client interface for the `system` service. Write a template interface that maps every method prototype in the remote `system` service. The template interface omits the detailed implementation of each method from the service, but keeps its annotations, return type, name of the method, list of arguments, as well as exception declarations.
...
<<<<<<< HEAD
The MicroProfile Rest Client feature automatically builds and generates an implementation for the `SystemClient` interface because the JAX-RS annotations in the interface tell the RESTful service
=======
...
In the `SystemClient` interface, the `getProperties()` method might throw an `UnknownUrlException` exception if the response from the `system` service is `404 NOT FOUND`.
The interface maps to what you want to invoke on the remote service. Then, the `@RegisterProvider(UnknownUrlExceptionMapper.class)` annotation registers the response exception mapper.
<<<<<<< HEAD
Exception mappers map various response code from the remote service to throwable exceptions.
=======
Exception mappers map various response codes from the remote service to throwable exceptions.
>>>>>>> 873335477642d58f1db817e835d3c040a49711ab
...
<<<<<<< HEAD
Now, instantiate the `SystemClient` interface and use it in the `inventory` service. If you want to connect only with the default host name, you can easily instantiate the `SystemClient` with CDI annotations.
CDI injection makes things a lot simpler when it comes to bootstrapping the client.
=======
...
First, you need to tell the MicroProfile Rest Client API the base URL value for the `system` service endpoint.
You can configure the default base URL of the `SystemClient` instance by using MicroProfile Config. The feature has been enabled for you in both `server.xml` and `pom.xml` files.
...
<<<<<<< HEAD
By default, interfaces have a scope of `@Dependent` unless there is another scope defined on the interface.

The `@RegisterRestClient` annotation registers the interface as a RESTful client. The runtime create a CDI bean for every interface that is annotated with the `@RegisterRestClient` annotation.

The `@Dependent` and `@RegisterRestClient` annotations imply that the interface is manageable through CDI. You must have them in order to inject the client by CDI.

CDI also requires an CDI managed object to inject the `SystemClient` interface into. Since the `InventoryManager` class is another object that is managed by CDI, add the client here.
=======
By default, interfaces have a `@Dependent` scope unless another scope is defined on the interface.

The `@RegisterRestClient` annotation registers the interface as a RESTful client. The runtime creates a CDI bean for every interface that is annotated with the `@RegisterRestClient` annotation.

The `@Dependent` and `@RegisterRestClient` annotations imply that the interface is manageable through CDI. You must have them in order to inject the client by CDI.

CDI also requires a CDI managed object to inject the `SystemClient` interface. Since the `InventoryManager` class is another object that is managed by CDI, add the client here.
>>>>>>> 873335477642d58f1db817e835d3c040a49711ab
...
<<<<<<< HEAD
The `inventory` service might also want to connect with a host other than the default `localhost`. Then, it is not reasonable to configure a base URL that is not yet known.
Therefore, set the host name as a variable and build the client by using the `RestClientBuilder` method, and you can customize the base URL from the given host name attribute.
=======
...
<<<<<<< HEAD
The host name is provided as a parameter. This method first assembles the base URL consisting of the new host name.
=======
...
You just built and tested a RESTful client interface to invoke a remote service with MicroProfile Rest Client and Open Liberty.
