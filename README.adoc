...
1. Visit the `http://localhost:9080/inventory/systems/localhost` URL. It retrieves the system property information for `localhost` by invoking the `http://localhost:9080/system/properties` client.

2. Get your internal IP address first. Then, visit the `http://localhost:9080/inventory/systems/<your_hostname>` URL by replacing `<your_hostname>` to your IP address to retrieve your system properties by invoking the `http://{your_ip_address}:9080/system/properties` client.
