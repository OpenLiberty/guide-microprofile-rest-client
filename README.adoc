...
In this guide, you will explore both methods to handle two scenarios for providing a valid base URL:
 ...
 - When the base URL is not yet known and requires to be determined during the runtime, set the base URL as a variable. Build the client with the more verbose `RestClientBuilder` method.
...
Next, create a RESTful client interface for the `system` service. Write a template interface that maps the API of the remote `system` service.
The template interface describes the remote service that you want to access. The interface defines the resource to access as a method by mapping its annotations, return type, list of arguments, as well as exception declarations.
