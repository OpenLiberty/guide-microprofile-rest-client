...
To try out the application, first go to the `finish` directory, which contains the complete `inventory` application. Then, run the following
...
After you are done checking out the application, stop the Open Liberty server by simply typing `q` in the shell session and then press the `enter/return` key.
...
----
#Replace the `UnknownUrlException` class.#
...
[role="code_command hotspot file=2", subs="quotes"]
----
#Replace the `UnknownUrlExceptionMapper` class.#
...
Look at the [hotspot=builder]`getPropertiesWithGivenHostName()` method in the `src/main/java/io/openliberty/guides/inventory/InventoryManager.java` file.
...
The Open Liberty server was already started in developer mode at the beginning of the guide and all the changes have been automatically picked up.
...
Since we started Open Liberty in developer mode at the start of the guide, simply press `enter` to run the tests. You will see the following output:
...
When you are done checking out the service, stop the Open Liberty server by simply typing `q`
in the shell session where you ran development mode and then press the `enter/return` key.
