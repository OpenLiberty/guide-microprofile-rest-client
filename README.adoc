...
Now you need to create a RESTful client interface that represents the `system` service, call it the `system` client.
...
Note that the service will automatically build and generate an implementation for the `system` client interface, because the JAX-RS annotations in this interface tell the RESTful service
...
In the `system` client, you add a response exception mapper as a provider to map the `404 NOT FOUND` response code with the `UnknownUrlException`. Read the next section to see more details.
...
First, configure the default base URL of the `system` client using MicroProfile Config. The feature has been enabled for you in both `server.xml` and `pom.xml` files.
Create a configuration file `src/main/webapp/META-IF/microprofile-config.properties` and add a config property `<fullyQualifiedInterfaceName>/mp-rest/url`, which in this case is `io.openliberty.guides.inventory.client.SystemClient/mp-rest/url`.
...
`@Inject` and `@RestClient` implies that CDI injects an instance of the `system` client to the `InventoryManager` class.
...
The host name is provided as a parameter. This method first assembles the base URL consisted of the new host name, then, instantiates a `RestClientBuilder` with this new URL, and builds the `system` client.
