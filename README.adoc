...
The `system` service is accessible at the http://localhost:9080/system/properties[http://localhost:9080/system/properties^] URL. In this case, `localhost` is the host name.

The 'system' service returns specific system properties when a multi-valued query is made to the 
http://localhost:9080/system/properties/listproperties[http://localhost:9080/system/properties/listproperties^] URL. 
For example, the 
[http://localhost:9080/system/properties/listproperties?properties=awt.toolkit,file.encoding.pkg](http://localhost:9080/system/properties/listproperties?properties=awt.toolkit,file.encoding.pkg) query URL provides the `awt.toolkit` and `file.encoding.pkg` system properties.

The `inventory` microservice makes a request to the `system` microservice and
stores the system property information. 
To fetch and store all your system information, visit the http://localhost:9080/inventory/systems/localhost[http://localhost:9080/inventory/systems/localhost^] URL.

To fetch and store specific properties, specify the properties in the
http://localhost:9080/inventory/systems/localhost[http://localhost:9080/inventory/systems/localhost^] URL. 
For example, to fetch `awt.toolkit` and `file.encoding.pkg` system properties, visit the 
[http://localhost:9080/inventory/systems/localhost?listproperties=awt.toolkit&listproperties=file.encoding.pkg](http://localhost:9080/inventory/systems/localhost?listproperties=awt.toolkit&listproperties=file.encoding.pkg) URL.
...
When the [hotspot=getProperties file=2]`getProperties()` method is invoked, 
the [hotspot=SystemClient file=2]`SystemClient` instance sends a GET request to the `<baseUrl>/properties` endpoint, 
where `<baseUrl>` is the default base URL of the `system` service.
Similarly, when the [hotspot=getListProperties file=2]`getListProperties()` method is invoked, 
the [hotspot=SystemClient file=2]`SystemClient` instance sends a GET request to the `<baseUrl>/properties/listproperties` endpoint.
You will see how to configure the base URL in the next section.
...
Configure the default base URL with the MicroProfile Config feature. This feature is enabled for you in the [hotspot=mpConfig file=3]`server.xml` file.
...
The [hotspot=redirects file=0]`followRedirects` property is configured to [hotspot=redirects file=0]`true`.
If the RESTFUL resource is relocated, this property allows the client to automatically follow the redirect. 

The [hotspot=queryParamStyle file=0]`queryParamStyle` property allows multi-valued query parameters to be formatted such that they meet the requirements of the server. 
The `system` service expects the request to be a single key with a comma-seperated list of values such as the 
[http://localhost:9080/system/properties/listproperties?properties=awt.toolkit,file.encoding.pkg](http://localhost:9080/system/properties/listproperties?properties=awt.toolkit,file.encoding.pkg) URL. 
So, the [hotspot=queryParamStyle file=0]`queryParamStyle` property is configured to [hotspot=queryParamStyle file=0]`COMMA_SEPARATED`.
When you visit the 
[http://localhost:9080/inventory/systems/localhost?listproperties=awt.toolkit&listproperties=file.encoding.pkg](http://localhost:9080/inventory/systems/localhost?listproperties=awt.toolkit&listproperties=file.encoding.pkg) URL,
the client reformats the request before invoking the `system` service.
The client recieve requests as multiple key/value pairs such as `?listproperties=awt.toolkit&listproperties=file.encoding.pkg`, 
but it must format requests to `system` as a single key with a comma-separated list such as `?properties=awt.toolkit,file.encoding.pkg`.
...
The helper function invokes the `system` service by calling the [hotspot=defaultRCGetProperties file=2]`defaultRestClient.getProperties()`  and [hotspot=defaultRCGetListProperties file=2]`defaultRestClient.getListProperties(listproperties)` methods.
...
The client is configured to automatically follow redirects using the [hotspot=followRedirects]`followRedirects(true)` method. Redirects occur when a RESTful resource has been relocated.
The client is also configured to send requests to the `system` service with multi-valued query parameters as a single key with a comma-separated list of values.

Similarly, call the [hotspot=customRCGetProperties]`customRestClient.getProperties()` and [hotspot=customRCGetListProperties]`customRestClient.getListProperties(listproperties)` methods to invoke the `system` service.
