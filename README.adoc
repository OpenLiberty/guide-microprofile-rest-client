...
:inventory-service: http://localhost:9080/inventory/systems/{your_hostname}
:invoked-service: http://{your_hostname}:9080/system/properties
:property-url: http://localhost:9080/system
:test-link: http://localhost:9080/inventory/systems/localhost
...
* The `{inventory-service}` microservice is the `inventory` service that invokes the `{invoked-service}` microservice. In Windows, Mac OS, and Linux, get your fully qualified domain name (FQDN) by entering `hostname` from your terminal. Visit the URL by replacing `{your_hostname}` with your FQDN.
...
The MicroProfile Rest Client API is added as a dependency to your [hotspot=1-236 file=0]`pom.xml` file. Look for the dependency with the [hotspot=95 file=0]`mpRestClient` artifact ID.
...
The [hotspot=8 file=1]`mpRestClient` feature is also enabled in the [hotspot=1-15 file=1]`src/main/liberty/config/server.xml` file. This feature enables your Open Liberty server to use MicroProfile Rest Client to invoke RESTful microservices.
...
[role="code_command hotspot=17-22", subs="quotes"]
----
...
When the [hotspot=21 file=2]`getProperties()` method is invoked, the `SystemClient` instance sends a GET request to the `<baseUrl>/properties` endpoint.
...
[role="code_command hotspot=3-14", subs="quotes"]
----
...
[role="code_command hotspot=10-24", subs="quotes"]
----
...
[role="code_command hotspot=1-2", subs="quotes"]
----
...
The config property to add the base URL is `<fullyQualifiedInterfaceName>/mp-rest/url`.
In this case, add the config property `io.openliberty.guides.inventory.client.SystemClient/mp-rest/url`.
Configure this property to the default [hotspot=2 file=0]`{property-url}` base URL:
...
Inject the [hotspot=17-22 file=1]`SystemClient` interface into the `InventoryManager` class, which is another CDI managed bean. 

[role="code_command hotspot=24-98", subs="quotes"]
----
...
[hotspot=29 file=2]`@Inject` and [hotspot=30 file=2]`@RestClient` annotations inject an instance of the [hotspot=31 file=2]`SystemClient` called `defaultRestClient` to the `InventoryManager` class.

Since the [hotspot=24-101 file=2]`InventoryManager` class is [hotspot=23 file=2]`@ApplicationScoped`, and the [hotspot=17-22 file=1]`SystemClient` CDI bean maintains the same scope through the [hotspot=13 file=1]`@Dependent` annotation, the client is initialized once per application.

If the `hostname` parameter is `localhost`, the service runs the [hotspot=36 file=2]`getPropertiesWithDefaultHostName()` helper function to fetch system properties.
The helper function invokes the `system` service by calling the [hotspot=60 file=2]`defaultRestClient.getProperties()` method.
...
Look at the [hotspot=69-87]`getPropertiesWithGivenHostName()` method in the `src/main/java/io/openliberty/guides/inventory/InventoryManager.java` file.
...
Then, the method instantiates a [hotspot=74-77]`RestClientBuilder` builder with the new URL, registers the response exception mapper, and builds the `SystemClient` instance.

Similarly, call the [hotspot=78]`customRestClient.getProperties()` method to invoke the `system` service.
...
- Get your FQDN first. Then, visit the `{inventory-service}` URL by replacing `{your_hostname}` with your FQDN, which retrieves your system properties by invoking the `{invoked-service}` service.
...
[role="code_command hotspot=17-96", subs="quotes"]
----
Create a `RestClientTest` class.
...
The [hotspot=46-56]`testDefaultLocalhost()` test fetches and compares system properties from the `{test-link}` URL.

The [hotspot=58-73]`testRestClientBuilder()` test gets your IP address. Then, use your IP address as the host name to fetch your system properties and compare them.
