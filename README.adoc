...
In order to instantiate the `SystemClient`, you must provide a valid base URL.
You will define the default base URL with host name as `localhost` using CDI and MicroProfile Config method. For example, by default `http://localhost:9080/system` is the base URL of the client.
Then, you will set the base URL as a variable with the host name as a parameter, and build the RESTful client using the `RestClientBuilder` method.
