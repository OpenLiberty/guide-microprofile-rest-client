...
* The http://localhost:9080/inventory/systems/{your_hostname} microservice is the `inventory` service that invokes the `{invoked-service}` microservice. In Windows, Mac OS, and Linux, get your fully qualified domain name (FQDN) by entering `hostname` from your terminal. Visit the URL by replacing `{your_hostname}` with your FQDN.
...
The MicroProfile Rest Client API is added as a dependency to your [hotspot=1-236 file=0]`pom.xml` file. Look for the dependency with the [hotspot=93 file=0]`mpRestClient` artifact ID.
...
- Get your FQDN first. Then, visit the http://localhost:9080/inventory/systems/{your_hostname} URL by replacing `{your_hostname}` with your FQDN, which retrieves your system properties by invoking the `{invoked-service}` service.
...
The [hotspot=47-57]`testDefaultLocalhost()` test fetches and compares system properties from the `{test-link}` URL.
