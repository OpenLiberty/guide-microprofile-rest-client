...
The MicroProfile Rest Client API is added as a dependency to your [hotspot file=0]`pom.xml` file. Look for the dependency with the [hotspot=mp-RestClient file=0]`mpRestClient` artifact ID.
...
The [hotspot=mp-RestClient file=1]`mpRestClient` feature is also enabled in the [hotspot file=1]`src/main/liberty/config/server.xml` file. This feature enables your Open Liberty server to use MicroProfile Rest Client to invoke RESTful microservices.
...
When the [hotspot=Properties file=2]`getProperties()` method is invoked, the [hotspot=SystemClient file=2]`SystemClient` instance sends a GET request to the `<baseUrl>/properties` endpoint, where `<baseUrl>` is the default base URL of the `system` service. You will see how to configure the base URL in the next section.
...
In the [hotspot=SystemClient file=2]`SystemClient` interface, add a response exception mapper as a provider to map the `404` response code with the [hotspot=Properties file=2]`UnknownUrlException` exception.
...
Now, link the [hotspot=UnknownUrlException file=1]`UnknownUrlException` class with the corresponding response code through a `ResponseExceptionMapper` mapper class.

[role="code_command hotspot", subs="quotes"]
----
#Create the `UnknownUrlExceptionMapper file=2` class.#
...
The config property [hotspot=1 file=0]`io.openliberty.guides.inventory.client.SystemClient/mp-rest/url` is configured to the default [hotspot=Config file=0]`\http://localhost:9080/system` base URL.
...
Inject the [hotspot=SystemClient file=1]`SystemClient` interface into the [hotspot=InventoryManager file=2]`InventoryManager` class, which is another CDI managed bean. 
...
[hotspot=Inject file=2]`@Inject` and [hotspot=RestClient file=2]`@RestClient` annotations inject an instance of the [hotspot=default-SystemClient file=2]`SystemClient` called `defaultRestClient` to the [hotspot=InventoryManager file=2]`InventoryManager` class.

Since the [hotspot=InventoryManager file=2]`InventoryManager` class is [hotspot=ApplicationScoped file=2]`@ApplicationScoped`, and the [hotspot=SystemClient file=1]`SystemClient` CDI bean maintains the same scope through the [hotspot=Dependent file=1]`@Dependent` annotation, the client is initialized once per application.
...
The helper function invokes the `system` service by calling the [hotspot=defaultRestClient.getProperties file=2]`defaultRestClient.getProperties()` method.
...
Similarly, call the [hotspot=customRestClient-getProperties]`customRestClient.getProperties()` method to invoke the `system` service.
...
[source, Java, linenums, role='code_column hide_tags=copyright']
----
include::finish/src/test/java/it/io/openliberty/guides/client/RestClientTest.java[]
----
