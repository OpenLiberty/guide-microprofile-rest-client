...
For example, the 
http://localhost:9080/system/properties?properties=os.name,user.timezone["http://localhost:9080/system/properties?properties=os.name,user.timezone"^] query URL provides the `os.name` and `user.timezone` system properties.
...
For example, to fetch the `os.name` and `user.timezone` system properties, visit the 
http://localhost:9080/inventory/systems/localhost?properties=os.name&properties=user.timezone[http://localhost:9080/inventory/systems/localhost?properties=os.name&properties=user.timezone^] URL.
...
http://localhost:9080/system/properties?properties=os.name,user.timezone["http://localhost:9080/system/properties?properties=os.name,user.timezone"^] URL. 
...
The client recieves the request as multiple key/value pairs such as `?properties=os.name&properties=user.timezone`
and then it formats the request to `system` service as a single key with a comma-separated list such as `?properties=os.name,user.timezone`.
...
The client is also configured to send requests to the `system` service with multi-valued query parameters as a single key with a [hotspot=queryParam]'COMMA_SEPARATED' list of values.
