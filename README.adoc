...
The client recieves requests as multiple key/value pairs such as `?properties=os.name&properties=user.timezone`
and then it formats requests to `system` service as a single key with a comma-separated list such as `?properties=os.name,user.timezone`.
