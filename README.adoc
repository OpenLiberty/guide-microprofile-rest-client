...
The MicroProfile Rest Client API automatically generates an instance of the template interface based on its annotations.
...
When the base URL of the remote service is static and able to configure, you can inject the client with CDI easily. When the base URL is not yet known and requires to be determined during the runtime, use the more verbose `RestClientBuilder` method.

In this guide, you will explore both methods to handle two scenarios for providing a valid base URL:

 - With the CDI method, define the default base URL with the `localhost` host name in the configuration file.

 - With the `RestClientBuilder` method, set the base URL as a variable with the host name as a parameter.
...
Next, create a RESTful client interface for the `system` service. Write a template interface that maps the API of the remote `system` service. The template interface omits the detailed implementation of each method from the service, but keeps its annotations, return type, name of the method, list of arguments, as well as exception declarations.
...
Error handling is an important step to ensure the application can fail safely. If there is an error response when invoking the remote service, for instance, the service returns a `404 NOT FOUND` response code,
you need to handle it. First, you need to define an exception, and map the exception with the error response code. Then, register the exception mapper in the client interface.

Look at the client interface from the previous section, the `@RegisterProvider(UnknownUrlExceptionMapper.class)` annotation registers the `UnknownUrlExceptionMapper` response exception mapper.
...
You can configure the default base URL by using MicroProfile Config. The feature has been enabled for you in both `server.xml` and `pom.xml` files.
...
Look at the `SystemClient` interface from the previous section:
...
Inject the `SystemClient` interface to the `InventoryManager` class, which is another CDI managed bean. Create the `InventoryManager` class in the `src/main/java/io/openliberty/guides/inventory/InventoryManager.java` file:
[source, java, indent=0]
----
...
You just built and tested invoking a remote service using a template interface with MicroProfile Rest Client and Open Liberty.
