...
The MicroProfile Rest Client feature automatically builds and generates a client implementation based on what is defined in the [hotspot=17-22]`SystemClient` interface. Thus, you don't waste time on setting up the client and connecting with the remote service.

When the [hotspot=21]`getProperties()` method is invoked, the `SystemClient` instance sends a GET request to the `<baseUrl>/properties` endpoint.

The [hotspot=15]`@RegisterProvider` annotation tells the framework to register the provider classes to be used when the framework invokes the interface. You can add as many providers as necessary.
In the [hotspot=17=22]`SystemClient` interface, add a response exception mapper as a provider to map the `404` response code with the `UnknownUrlException` exception.
...
[role="code_command hotspot=23-100", subs="quotes"]
----
...
[role="code_command hotspot=16-96", subs="quotes"]
----
