...
= Consuming RESTful microservices in a type-safe way
...
service on that host. The `system` service simulates a remote service in this application.

You will learn how to create and register the client interface and handle exceptions through `ResponseExceptionMappers`.
Then, you will instantiate the client and use it in the `inventory` service. It is possible to do so using two different approaches: CDI with the help of MicroProfile Config, and the `RestClientBuilder`.
...
You will handle various scenarios in the following way:
 ...
 - Define the default base URL with host name as `localhost` in the configuration file, and inject the client using the CDI method.

 - Set the base URL as a variable with the host name as a parameter, and build the RESTful client using the `RestClientBuilder` method.
...
Now you need to create a RESTful client interface that represents the `system` service, call it the `SystemClient`.
...
The service automatically builds and generates an implementation for the `SystemClient` interface, because the JAX-RS annotations in this interface tell the RESTful service
what to do. That allows you to not have to worry about all of the boilerplate code (like setting up a client class, connecting to the remote server, invoking the right URI with the right parameters).

When the `getProperties()` method gets invoked, the `SystemClient` sends a GET request to the endpoint at `<baseUrl>/properties`.

The `@Dependent` annotation tells the service the scope of this interface.

The `@RegisterRestClient` annotation registers this interface as a RESTful client.

The `@RegisterProvider` annotation tells the framework to register provider classes to be used when invoking this interface; you can add as many providers as necessary.
In the `SystemClient`, you add a response exception mapper as a provider to map the `404 NOT FOUND` response code with the `UnknownUrlException`.
...
In the `SystemClient` interface, the `getProperties()` method may throw an `UnknownUrlException` if the response from the `system` service is `404 NOT FOUND`, and `@RegisterProvider(UnknownUrlExceptionMapper.class)` registers the response exception mapper.
...
Now, you need to link the `UnknownUrlException` class with the corresponding response code through a `ResponseExceptionMapper`.
...
Now you need to instantiate the `SystemClient` class and use it in the `inventory` service. If you only need to connect with the default host name, it is easy to instantiate the `SystemClient` using CDI annotations.

First, configure the default base URL of the `SystemClient` using MicroProfile Config. The feature has been enabled for you in both `server.xml` and `pom.xml` files.
...
Since `InventoryManager` class is another CDI managed object (CDI requires one to inject the `SystemClient`), you add the client here.

If the parameter `hostname` is `localhost`, then the service runs the `getPropertiesWithDefaultHostName()` helper function to fetch system properties.
...
The `inventory` service may also provide a different host name rather than the default `localhost`. Set the host name as a variable and build the client using the `RestClientBuilder`.
...
The host name is provided as a parameter. This method first assembles the base URL consisted of the new host name, then, instantiates a `RestClientBuilder` with the new URL, registers the response exception mapper, and builds the `SystemClient`.
...
2. Get your internal IP address first. Then, visit `http://localhost:9080/inventory/systems/{ip_address}` to retrieve your system properties by invoking the `http://{ip_address}:9080/system/properties` client.
...
To see whether the tests detect a failure, change the path of the `system` service, so that when the `inventory` service tries to access the invalid URL, a `404 NOT FOUND` exception is thrown.
