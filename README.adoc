...
* Get your fully qualified domain name (FQDN) first, then visit the `http://localhost:9080/inventory/systems/{your_hostname}` URL by replacing `{your_hostname}` with your FQDN.

You will see the same system property information, but the process of getting this information is actually implemented differently.
...
2. Get your fully qualified domain name (FQDN) first. Then, visit the `http://localhost:9080/inventory/systems/{your_hostname}` URL by replacing `{your_hostname}` with your FQDN. It retrieves your system properties by invoking the `http://{your_hostname}:9080/system/properties` service.
